...
{
    internal static class BackupCursorCodec
    {
        public static string Encode(BackupCursor cursor)
        {
        ...
        public static BackupCursor Decode(string value)
        {
            if (string.IsNullOrWhiteSpace(value))
                throw new FormatException("invalid_backup_cursor");
            try
            {
                ...
                return new BackupCursor(DateTimeOffset.FromUnixTimeMilliseconds(milliseconds), id);
            }
            catch (Exception exception) when (
                exception is FormatException || exception is ArgumentException ||
                exception is OverflowException)
            {
                throw new FormatException("invalid_backup_cursor", exception);
            }
        }
    }
}
