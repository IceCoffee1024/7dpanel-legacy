...
namespace LSTY.SevenDPanel.Adapters.Web.Inbound.Http.OpenApi
{
    internal sealed class PanelOpenApiOperationProcessor : IOperationProcessor
    {
        ...
        private static void DescribeEvidenceFoundation(OperationProcessorContext context)
        {
            var operation = context.OperationDescription.Operation;
            var path = context.OperationDescription.Path;
            var method = context.OperationDescription.Method;
            ...
            if (string.Equals(path, "/api/v1/chat/mutes", StringComparison.Ordinal))
            {
                ...
                else if (method == OpenApiOperationMethod.Post)
                {
                    ...
                    MoveSuccessResponse(operation, "200", "201", "The chat mute was created.");
                }
                return;
            }
            ...
            else if (method == OpenApiOperationMethod.Delete)
            {
                ...
                MoveSuccessResponse(operation, "200", "204", "The chat mute was released.");
            }
        }

        private static void DescribePlayerEvidenceActions(OperationProcessorContext context)
        {
            var path = context.OperationDescription.Path;
            var method = context.OperationDescription.Method;
            var operation = context.OperationDescription.Operation;
        ...
        private static void DescribeJobsBackupsSchedules(OperationProcessorContext context)
        {
            ...
            if (path.StartsWith("/api/v1/jobs", StringComparison.Ordinal))
            {
                ...
                else if (method == OpenApiOperationMethod.Post)
                {
                    ...
                    MoveSuccessResponse(operation, "200", "202", "The cancellation request was accepted.");
                }
                return;
            }
            ...
            if (path.StartsWith("/api/v1/backups", StringComparison.Ordinal))
            {
                ...
                else if (string.Equals(path, "/api/v1/backups/world", StringComparison.Ordinal))
                {
                    ...
                    MoveSuccessResponse(operation, "200", "202", "The backup job was accepted.");
                }
                ...
                else if (path.EndsWith("/restore", StringComparison.Ordinal))
                {
                    ...
                    MoveSuccessResponse(operation, "200", "202", "The restore was staged.");
                }
                else if (method == OpenApiOperationMethod.Delete)
                {
                    operation.OperationId = "deleteBackup";
                    MoveSuccessResponse(operation, "200", "204", "The backup was deleted.");
                }
                return;
            }
                ...
                method == OpenApiOperationMethod.Post)
            {
                ...
                RemoveParameter(operation, "cancellationToken");
                MoveSuccessResponse(operation, "200", "202", "The announcement was accepted.");
                return;
            }
            ...
            if (string.Equals(path, "/api/v1/schedules", StringComparison.Ordinal))
            {
                ...
                else if (method == OpenApiOperationMethod.Post)
                {
                    ...
                    MoveSuccessResponse(operation, "200", "201", "The schedule was created.");
                }
                return;
            }
        ...
        private static void RequireSchemaProperties(
