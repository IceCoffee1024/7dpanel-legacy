...
function failure(error: unknown, auth: Auth, stale: boolean): { state: CommerceState, code: string } {
...
function useRunner(auth: Auth) {
  const state = shallowRef<CommerceState>('idle')
  ...
  function run<T>(action: (token: string, signal: AbortSignal) => Promise<T>, apply: (value: T) => void, mutating: boolean, hasData: () => boolean): Promise<boolean> {
    ...
    const pending = action(token, current.signal).then((value) => {
      ...
      if (disposed || current.signal.aborted)
        return false
      const result = failure(error, auth, hasData())
...
export function useShopProducts(options: { auth?: Auth } = {}): ShopController {
  ...
  const runner = useRunner(auth)
  const product = shallowRef<ShopProduct | null>(null)
  const purchase = shallowRef<ShopPurchase | null>(null)
  return {
    state: readonly(runner.state),
    isMutating: readonly(runner.isMutating),
    errorCode: readonly(runner.errorCode),
    product: readonly(product),
    purchase: readonly(purchase),
    load: id => runner.run((token, signal) => api.fetchShopProduct(token, id, signal), (value) => {
      product.value = value
    }, false, () => product.value !== null),
    save: draft => runner.run((token, signal) => api.saveShopProduct(token, draft, signal), (value) => {
      product.value = value
    }, true, () => product.value !== null),
    purchaseProduct: input => runner.run((token, signal) => api.purchaseShopProduct(token, input, signal), (value) => {
      purchase.value = value.purchase
    }, true, () => purchase.value !== null),
...
export function useRedeemCodes(options: { auth?: Auth } = {}): RedeemCodesController {
  const auth = options.auth ?? useAuthStore()
  const runner = useRunner(auth)
  const definition = shallowRef<RedeemCodeDefinition | null>(null)
  const generated = shallowRef<GeneratedRedeemCode | null>(null)
  return {
    state: readonly(runner.state),
    isMutating: readonly(runner.isMutating),
    errorCode: readonly(runner.errorCode),
    ...
    load: id => runner.run((token, signal) => api.fetchRedeemCode(token, id, signal), (value) => {
      definition.value = value
    }, false, () => definition.value !== null),
    create: input => runner.run((token, signal) => api.createRedeemCode(token, input, signal), (value) => {
      generated.value = value
      definition.value = value.definition
    }, true, () => definition.value !== null),
    clearGenerated: () => {
...
export function useAchievementOnlineRewards(options: { auth?: Auth } = {}): AchievementOnlineController {
  const auth = options.auth ?? useAuthStore()
  const runner = useRunner(auth)
  const achievement = shallowRef<AchievementDefinition | null>(null)
  const achievementRecord = shallowRef<AchievementRecord | null>(null)
  const rule = shallowRef<OnlineRewardRule | null>(null)
  const records = shallowRef<readonly OnlineRewardRecord[]>(Object.freeze([]))
  ...
  return {
    ...
    achievementRecord: readonly(achievementRecord),
    rule: readonly(rule),
    records: readonly(records),
    saveAchievement: draft => runner.run((token, signal) => api.saveAchievementDefinition(token, draft, signal), (value) => {
      achievement.value = value
    }, true, hasData),
    loadAchievementRecord: (id, player) => runner.run((token, signal) => api.fetchAchievementRecord(token, id, player, signal), (value) => {
      achievementRecord.value = value
    }, false, hasData),
    saveRule: draft => runner.run((token, signal) => api.saveOnlineRewardRule(token, draft, signal), (value) => {
      rule.value = value
    }, true, hasData),
    loadRecords: (id, player) => runner.run((token, signal) => api.fetchOnlineRewardRecords(token, id, player, signal), (value) => {
      records.value = Object.freeze([...value])
      runner.state.value = value.length === 0 ? 'empty' : 'fresh'
    }, false, hasData),
    manualGrant: input => runner.run((token, signal) => api.grantManualOnlineReward(token, input, signal), (value) => {
      records.value = Object.freeze([value, ...records.value.filter(item => item.eligibilityId !== value.eligibilityId)])
    }, true, hasData),
    dispose: runner.dispose,
  }
}
