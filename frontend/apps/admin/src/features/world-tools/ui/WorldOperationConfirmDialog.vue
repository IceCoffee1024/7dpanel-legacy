  ...
  >
    <template #body>
      <div v-if="props.review" class="space-y-4">
        ...
        <dl class="grid gap-3 rounded-lg border border-default p-4 sm:grid-cols-2">
          <div>
            <dt class="text-xs font-medium text-muted">
              {{ t('worldTools.confirm.target') }}
            </dt><dd class="break-words text-sm text-highlighted">
              {{ props.review.target }}
            </dd>
          </div>
          <div>
            <dt class="text-xs font-medium text-muted">
              {{ t('worldTools.confirm.world') }}
            </dt><dd class="break-all text-sm text-highlighted">
              {{ props.review.worldId }}
            </dd>
          </div>
          <div class="sm:col-span-2">
            <dt class="text-xs font-medium text-muted">
              {{ t('worldTools.confirm.scope') }}
            </dt><dd class="break-words text-sm text-highlighted">
              {{ props.review.scope }}
            </dd>
          </div>
          <div>
            <dt class="text-xs font-medium text-muted">
              {{ t('worldTools.confirm.worldVersion') }}
            </dt><dd class="break-all text-sm text-highlighted">
              {{ props.review.worldVersion }}
            </dd>
          </div>
          <div>
            <dt class="text-xs font-medium text-muted">
              {{ t('worldTools.confirm.mapResourceVersion') }}
            </dt><dd class="break-all text-sm text-highlighted">
              {{ props.review.mapResourceVersion ?? '—' }}
            </dd>
          </div>
          <div v-if="props.review.catalogVersion" class="sm:col-span-2">
            <dt class="text-xs font-medium text-muted">
              {{ t('worldTools.confirm.catalogVersion') }}
            </dt><dd class="break-all text-sm text-highlighted">
              {{ props.review.catalogVersion }}
            </dd>
          </div>
          <div class="sm:col-span-2">
            <dt class="text-xs font-medium text-muted">
              {{ t('worldTools.confirm.expectedImpact') }}
            </dt><dd class="text-sm text-highlighted">
              {{ props.review.impact }}
            </dd>
          </div>
          <div class="sm:col-span-2">
            <dt class="text-xs font-medium text-muted">
              {{ t('worldTools.confirm.reversibility') }}
            </dt><dd class="text-sm font-medium" :class="props.review.reversible ? 'text-success' : 'text-warning'">
              {{ props.review.reversible ? t('worldTools.confirm.reversible') : t('worldTools.confirm.notReversible') }}
            </dd>
          </div>
        </dl>
        ...
        >
          <UInput
            v-model="strongConfirmationText"
            autocomplete="off"
            class="w-full"
            placeholder="CONFIRM"
          />
