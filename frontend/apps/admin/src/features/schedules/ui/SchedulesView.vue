...
function remove(schedule: ScheduleRecord) {
  if (window.confirm(t('schedules.delete.confirm', { name: schedule.name })))
...
<template>
  <UDashboardPanel id="schedules">
    <template #header>
      <UDashboardNavbar :title="t('schedules.title')">
        ...
        <template #right>
          <UButton color="neutral" :disabled="controller.isMutating.value" icon="i-lucide-refresh-cw" :label="t('common.reload')" variant="outline" @click="controller.refresh" />
        </template>
      </UDashboardNavbar>
    </template>
    ...
    <template #body>
      <UContainer class="space-y-5 py-5">
        ...
        <UAlert v-if="controller.errorCode.value" color="error" :description="controller.errorCode.value" :title="t('schedules.state.operationFailed')" />
        ...
        <UCard>
          <template #header>
            <div class="flex flex-wrap items-center justify-between gap-3">
              <div>
                <h2 class="font-semibold">{{ t('schedules.catalog.title') }}</h2>
                <p class="text-sm text-muted">{{ t('schedules.catalog.description') }}</p>
              </div>
              <UButton :disabled="controller.isMutating.value" icon="i-lucide-plus" :label="t('schedules.form.create')" @click="openCreate" />
            </div>
          </template>
          <ScheduleTable
  ...
  <UModal v-model:open="formOpen" :description="t('schedules.form.description')" :title="editing === null ? t('schedules.form.create') : t('schedules.form.edit')">
    <template #body>
      <ScheduleForm :disabled="controller.isMutating.value" :schedule="editing" @cancel="formOpen = false" @submit="save" />
    </template>
  </UModal>
</template>
