...
const removeTarget = shallowRef<
  | { kind: 'ban', entry: BanEntry }
  | { kind: 'whitelist', entry: WhitelistEntry }
  | null
>(null)
...
function editBan(entry: BanEntry) {
  selectedBan.value = entry
  banDialogOpen.value = true
}
...
function editWhitelist(entry: WhitelistEntry) {
  selectedWhitelist.value = entry
  whitelistDialogOpen.value = true
}
async function saveBan(input: BanInput) {
  if (await controller.saveBan(input))
    banDialogOpen.value = false
}
async function saveWhitelist(input: WhitelistInput) {
  if (await controller.saveWhitelist(input))
    whitelistDialogOpen.value = false
}
function removeBan(entry: BanEntry) {
  removeTarget.value = { kind: 'ban', entry }
}
function removeWhitelist(entry: WhitelistEntry) {
  removeTarget.value = { kind: 'whitelist', entry }
}
function confirmRemove() {
  ...
  if (target.kind === 'ban')
    void controller.removeBan(target.entry.playerId)
  else
    void controller.removeWhitelist(target.entry.playerId)
}
...
<template>
      ...
      <div class="flex flex-wrap items-center justify-between gap-3 p-4">
        <div>
          <h1 class="text-lg font-semibold">
            ...
            {{ t('accessLists.description') }}
          </p>
        </div>
        <div class="flex gap-2">
          <UButton :label="t('accessLists.tabs.bans')" :variant="tab === 'ban' ? 'solid' : 'outline'" @click="selectTab('ban')" /><UButton :label="t('accessLists.tabs.whitelist')" :variant="tab === 'whitelist' ? 'solid' : 'outline'" @click="selectTab('whitelist')" />
        </div>
      </div>
    </template>
    <template #body>
      <div class="space-y-4 p-4">
        ...
        <UAlert
          ...
          color="neutral"
          :title="t('accessLists.state.readOnlyTitle')"
          :description="t('accessLists.state.readOnlyDescription')"
        />
        <div class="flex justify-end">
          <UButton
            v-if="tab === 'ban' && controller.canMutate.value"
            data-testid="add-ban"
            :label="t('accessLists.action.addBan')"
            @click="addBan"
          />
          <UButton
            v-if="tab === 'whitelist' && controller.canMutate.value"
            data-testid="add-whitelist"
            :label="t('accessLists.action.addWhitelist')"
            @click="addWhitelist"
          />
        </div>
          ...
          <article v-for="entry in visibleBans" :key="entry.playerId" class="flex items-center justify-between gap-3 rounded-lg border border-default p-3">
            <div>
              <p class="font-medium">
                ...
                {{ entry.playerId }} · {{ entry.bannedUntilUtc ?? t('accessLists.permanent') }}
              </p><p v-if="entry.reason" class="text-sm">
                {{ entry.reason }}
              </p>
            </div>
            <div v-if="controller.canMutate.value" class="flex gap-2">
              <UButton
                :data-testid="`edit-ban-${entry.playerId}`"
                :label="t('common.edit')"
                variant="outline"
                @click="editBan(entry)"
              /><UButton
                :label="t('accessLists.action.remove')"
                color="error"
                variant="outline"
                @click="removeBan(entry)"
              />
            </div>
          </article>
          <p v-if="visibleBans.length === 0" class="text-sm text-muted">
            {{ t('accessLists.empty.bans') }}
          </p>
        </div>
        <div v-else class="space-y-2">
          <article v-for="entry in visibleWhitelist" :key="entry.playerId" class="flex items-center justify-between gap-3 rounded-lg border border-default p-3">
            ...
            <div v-if="controller.canMutate.value" class="flex gap-2">
              <UButton
                :data-testid="`edit-whitelist-${entry.playerId}`"
                :label="t('common.edit')"
                variant="outline"
                @click="editWhitelist(entry)"
              /><UButton
                :label="t('accessLists.action.remove')"
                color="error"
                variant="outline"
          ...
          <p v-if="visibleWhitelist.length === 0" class="text-sm text-muted">
            {{ t('accessLists.empty.whitelist') }}
          </p>
        </div>
      </div>
    </template>
  ...
  <UModal
    ...
    :description="removeTarget?.kind === 'ban'
      ? t('accessLists.confirm.removeBan', { name: removeTarget.entry.displayName || removeTarget.entry.playerId })
      : removeTarget?.kind === 'whitelist'
  ...
  >
    <template #footer>
      <UButton
        ...
        @click="closeRemove"
      />
      <UButton color="error" :label="t('common.confirm')" @click="confirmRemove" />
    </template>
  </UModal>
</template>
