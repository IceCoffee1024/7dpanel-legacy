...
const api = vi.hoisted(() => ({
  createDiscordBindingCode: vi.fn(), deleteDiscordBinding: vi.fn(), getDiscordConfiguration: vi.fn(), getDiscordHealth: vi.fn(),
  listDiscordBindings: vi.fn(), listDiscordCommands: vi.fn(), listDiscordDeliveries: vi.fn(), retryDiscordDelivery: vi.fn(),
  saveDiscordConfiguration: vi.fn(), testDiscordDelivery: vi.fn(), updateDiscordSecret: vi.fn(),
}))
...
const configuration = Object.freeze({
  version: 1, isEnabled: true, mode: 'Bot', applicationId: 'app', guildId: 'guild', publicChannelId: 'channel',
  bridgeGameToDiscord: true, bridgeDiscordToGame: true, proxy: { isEnabled: false, endpoint: null, hasCredentials: false },
  hasBotToken: true, targets: [], updatedAtUtc: '2026-07-27T00:00:00Z',
})
...
function mountComposable() {
  ...
  const app = createApp({ setup() { result = useDiscord(); return () => null } })
...
describe('useDiscord', () => {
  ...
  beforeEach(() => {
    ...
    api.listDiscordBindings.mockResolvedValue([]); api.listDiscordCommands.mockResolvedValue([]); api.listDiscordDeliveries.mockResolvedValue([])
  })
  ...
  it('aborts all in-flight sections and exposes stable 403 errors', async () => {
    ...
    api.getDiscordConfiguration.mockImplementation((_auth: string, signal: AbortSignal) => { signals.push(signal); return Promise.resolve(configuration) })
    api.getDiscordHealth.mockImplementation((_auth: string, signal: AbortSignal) => { signals.push(signal); return pending })
    api.listDiscordDeliveries.mockImplementation((_auth: string, signal: AbortSignal) => { signals.push(signal); return pending })
    api.listDiscordBindings.mockImplementation((_auth: string, signal: AbortSignal) => { signals.push(signal); return pending })
    api.listDiscordCommands.mockImplementation((_auth: string, signal: AbortSignal) => { signals.push(signal); return pending })
    const mounted = mountComposable(); apps.push(mounted.app)
    await flushPromises()
