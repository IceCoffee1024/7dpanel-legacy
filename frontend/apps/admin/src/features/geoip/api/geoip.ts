...
function parseProvider(value: unknown): GeoIpProvider {
  return typeof value === 'string' && providers.has(value as GeoIpProvider) ? value as GeoIpProvider : invalid()
}
function parseMode(value: unknown): GeoIpFailureMode {
  return typeof value === 'string' && modes.has(value as GeoIpFailureMode) ? value as GeoIpFailureMode : invalid()
}
function parseEffect(value: unknown): GeoIpEffect {
  return typeof value === 'string' && effects.has(value as GeoIpEffect) ? value as GeoIpEffect : invalid()
}
function parseSeverity(value: unknown): GeoIpDiagnosticSeverity {
  return typeof value === 'string' && severities.has(value as GeoIpDiagnosticSeverity) ? value as GeoIpDiagnosticSeverity : invalid()
}
function parseLookupStatus(value: unknown): GeoIpLookupStatus | null {
  return value === null ? null : typeof value === 'string' && lookupStatuses.has(value as GeoIpLookupStatus) ? value as GeoIpLookupStatus : invalid()
}
function parseCredential(value: unknown): GeoIpCredentialState {
  const source = record(value)
  keys(source, ['isSet', 'fingerprint', 'updatedAtUtc'])
  ...
  if (isSet !== (fingerprint !== null && updatedAtUtc !== null))
    invalid()
...
function parseProviders(value: unknown): readonly GeoIpProviderMetadata[] {
  ...
  return Object.freeze(value.map((entry) => {
    ...
    keys(item, ['provider', 'isExternal', 'dataVersion', 'buildEpoch'])
    return Object.freeze({ provider: parseProvider(item.provider), isExternal: bool(item.isExternal), dataVersion: nullableText(item.dataVersion), buildEpoch: nullableText(item.buildEpoch) })
  }))
}
...
export function parseGeoIpPolicy(value: unknown): GeoIpPolicy {
  ...
  if (!Array.isArray(source.networkRules) || !Array.isArray(source.countryRules) || !Array.isArray(source.recentDecisions))
    invalid()
  const cache = record(source.cacheHealth)
  keys(cache, ['queueDepth', 'rejectedRefreshCount', 'lastCompletedAtUtc', 'lastLookupStatus', 'severity', 'statusCode'])
  return Object.freeze({
    version: integer(source.version),
    isEnabled: bool(source.isEnabled),
    provider: parseProvider(source.provider),
    failureMode: parseMode(source.failureMode),
    bypassAdmins: bool(source.bypassAdmins),
    rejectionMessage: text(source.rejectionMessage),
    networkRules: Object.freeze(source.networkRules.map((entry) => {
      const item = record(entry)
      keys(item, ['ruleId', 'networkCidr', 'effect', 'ordinal'])
      return Object.freeze({ ruleId: text(item.ruleId), networkCidr: text(item.networkCidr), effect: parseEffect(item.effect), ordinal: integer(item.ordinal) })
    })),
    countryRules: Object.freeze(source.countryRules.map((entry) => {
      const item = record(entry)
      keys(item, ['countryCode', 'effect'])
      return Object.freeze({ countryCode: text(item.countryCode), effect: parseEffect(item.effect) })
    })),
    ...
    recentDecisions: Object.freeze(source.recentDecisions.map((entry) => {
      const item = record(entry)
      keys(item, ['occurredAtUtc', 'maskedIp', 'decision', 'reasonCode', 'lookupStatus'])
      return Object.freeze({ occurredAtUtc: nullableUtc(item.occurredAtUtc) ?? invalid(), maskedIp: text(item.maskedIp), decision: text(item.decision), reasonCode: text(item.reasonCode), lookupStatus: text(item.lookupStatus) })
    })),
  })
}
...
export function parseGeoIpCredentials(value: unknown): GeoIpCredentials {
  const source = record(value)
  keys(source, ['accountId', 'licenseKey'])
  return Object.freeze({ accountId: parseCredential(source.accountId), licenseKey: parseCredential(source.licenseKey) })
}

function headers(auth: string, json = false): HeadersInit {
  return { Authorization: auth, ...(json ? { 'Content-Type': 'application/json' } : {}) }
}
export async function getGeoIpPolicy(auth: string, signal?: AbortSignal): Promise<GeoIpPolicy> {
  return parseGeoIpPolicy(await requestJson<unknown>('/api/v1/access-policies/geoip', { headers: headers(auth), signal }))
}
export async function saveGeoIpPolicy(auth: string, draft: GeoIpPolicyDraft, signal?: AbortSignal): Promise<void> {
  await requestJson('/api/v1/access-policies/geoip', { method: 'PUT', headers: headers(auth, true), body: JSON.stringify(draft), signal })
}
export async function testGeoIpPolicy(auth: string, ipAddress: string, signal?: AbortSignal): Promise<GeoIpTestResult> {
  const source = record(await requestJson<unknown>('/api/v1/access-policies/geoip/test', { method: 'POST', headers: headers(auth, true), body: JSON.stringify({ ipAddress }), expectedStatus: 202, signal }))
  keys(source, ['accepted', 'maskedIp', 'state'])
...
export async function getGeoIpDiagnostics(auth: string, signal?: AbortSignal): Promise<GeoIpDiagnostics> {
  const source = record(await requestJson<unknown>('/api/v1/access-policies/geoip/diagnostics', { headers: headers(auth), signal }))
  keys(source, ['isEnabled', 'failureMode', 'provider', 'severity', 'statusCode', 'queueDepth', 'rejectedRefreshCount', 'lastCompletedAtUtc', 'lastLookupStatus', 'providers'])
