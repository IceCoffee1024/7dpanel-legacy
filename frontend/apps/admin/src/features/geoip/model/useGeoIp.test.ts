...
function mountComposable() {
  let result!: ReturnType<typeof useGeoIp>
...
describe('useGeoIp', () => {
  ...
  beforeEach(() => {
    ...
    api.getGeoIpPolicy.mockResolvedValue(policy)
    api.getGeoIpDiagnostics.mockResolvedValue(diagnostics('ready'))
    api.getGeoIpCredentials.mockResolvedValue(credentials)
  })
  afterEach(() => {
  ...
  it('aborts policy, diagnostics, credential reads, and credential mutations and exposes stable 403 errors', async () => {
    ...
    api.getGeoIpPolicy.mockImplementation((_auth: string, signal: AbortSignal) => {
      signals.push(signal)
      return Promise.resolve(policy)
    })
    api.getGeoIpDiagnostics.mockImplementation((_auth: string, signal: AbortSignal) => {
      signals.push(signal)
      return pending
    })
    api.getGeoIpCredentials.mockImplementation((_auth: string, signal: AbortSignal) => {
      signals.push(signal)
      return Promise.resolve(credentials)
    })
    api.updateGeoIpCredentials.mockImplementation((_auth: string, _draft: unknown, signal: AbortSignal) => {
      signals.push(signal)
      return pending
    })
    const mounted = mountComposable()
