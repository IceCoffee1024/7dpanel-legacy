      ...
      <UDashboardNavbar :title="t('economy.accounts.title')">
        ...
        <template #right>
          <UButton
            ...
            :label="t('economy.common.refresh')"
            variant="outline"
            :loading="props.controller.isLoading.value"
            @click="submitSearch"
          />
        </template>
      </UDashboardNavbar>
    ...
    <template #body>
      <UContainer class="space-y-5 py-5">
        <UAlert
          ...
          color="warning"
          :title="t('economy.accounts.state.stale')"
        ...
        <UAlert
          ...
          color="error"
          :title="t(props.controller.state.value === 'forbidden' ? 'economy.accounts.state.forbidden' : 'economy.accounts.state.unavailable')"
          :description="props.controller.errorCode.value ?? undefined"
        />
        <UAlert
          v-if="props.controller.errorCode.value && props.controller.state.value === 'fresh'"
          color="error"
          :title="t('economy.common.operationIncomplete')"
          :description="props.controller.errorCode.value"
        />

        <UCard>
          <div class="grid gap-3 md:grid-cols-[minmax(0,1fr)_auto_auto] md:items-end">
            <UFormField :label="t('economy.accounts.search.label')">
              <UInput
                v-model="search"
                class="w-full"
                :placeholder="t('economy.accounts.search.placeholder')"
        ...
        <UCard v-else-if="props.controller.accounts.value.length === 0">
          <p class="text-sm text-muted">
            {{ t('economy.accounts.state.empty') }}
          </p>
        </UCard>
        <div v-else class="grid gap-3 xl:grid-cols-2">
          <UCard v-for="account in props.controller.accounts.value" :key="account.accountId">
              ...
              <div class="flex flex-wrap items-center justify-between gap-2">
                <div>
                  <p class="font-semibold text-highlighted">
                    {{ account.crossplatformId ?? account.accountId }}
                  </p><p class="text-xs text-muted">
                    {{ account.accountId }}
                  </p>
                </div>
                <UBadge :color="badgeColor(account)" variant="subtle">
                  {{ t(!account.enabled ? 'economy.accounts.status.disabled' : account.isFrozen ? 'economy.accounts.status.frozen' : 'economy.accounts.status.normal') }}
                </UBadge>
              </div>
            </template>
            <dl class="grid grid-cols-3 gap-3 text-sm">
              <div>
                <dt class="text-muted">
                  {{ t('economy.accounts.balance.posted') }}
                </dt><dd class="mt-1 font-medium">
                  {{ formatEconomyAmount(account.postedBalance) }}
                </dd>
              </div>
              <div>
                <dt class="text-muted">
                  {{ t('economy.accounts.balance.reservedDebit') }}
                </dt><dd class="mt-1 font-medium">
                  {{ formatEconomyAmount(account.reservedDebit) }}
                </dd>
              </div>
              <div>
                <dt class="text-muted">
                  {{ t('economy.accounts.balance.available') }}
                </dt><dd class="mt-1 font-medium">
                  {{ formatEconomyAmount(account.availableBalance) }}
                </dd>
              </div>
            </dl>
            <template #footer>
              <div class="flex justify-end">
                <UButton
                  v-if="account.kind === 'Player'"
                  :color="account.isFrozen ? 'success' : 'warning'"
                  :label="t(account.isFrozen ? 'economy.accounts.action.unfreeze' : 'economy.accounts.action.freeze')"
                  variant="outline"
                  :loading="props.controller.mutationAccountId.value === account.accountId"
                  @click="emit('setFrozen', account, !account.isFrozen)"
                />
              </div>
            </template>
          </UCard>
        </div>
        <div v-if="props.controller.nextCursor.value" class="flex justify-center">
          <UButton
            color="neutral"
            :label="t('economy.common.loadMore')"
            variant="outline"
            :loading="props.controller.isLoading.value"
            @click="emit('loadNext')"
          />
          ...
          <template #header>
            <div>
              <h2 class="font-semibold">
                {{ t('economy.accounts.adjustment.title') }}
              </h2><p class="text-sm text-muted">
                {{ t('economy.accounts.adjustment.description') }}
              </p>
            </div>
          </template>
          <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            <UFormField :label="t('economy.accounts.adjustment.playerId')">
              <UInput v-model="crossplatformId" class="w-full" />
            </UFormField>
            <UFormField :label="t('economy.accounts.adjustment.direction')">
              <USelect v-model="side" class="w-full" :items="sideItems" />
            </UFormField>
            <UFormField :label="t('economy.accounts.adjustment.amount')">
              <UInput v-model="amount" class="w-full" inputmode="numeric" />
            </UFormField>
            <UFormField :label="t('economy.accounts.adjustment.reason')">
              <UInput v-model="reason" class="w-full" />
            </UFormField>
          </div>
          <template #footer>
            <div class="flex justify-end">
              <UButton :label="t('economy.accounts.adjustment.submit')" :disabled="!adjustmentValid || props.controller.mutationAccountId.value !== null" @click="submitAdjustment" />
            </div>
          </template>
        </UCard>
      </UContainer>
