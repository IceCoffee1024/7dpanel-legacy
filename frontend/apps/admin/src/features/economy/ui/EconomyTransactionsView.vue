    ...
    <template #body>
      <UContainer class="space-y-5 py-5">
        <UAlert
          ...
          color="warning"
          :title="t('economy.transactions.state.stale')"
        ...
        <UAlert
          ...
          color="error"
          :title="t(props.controller.state.value === 'forbidden' ? 'economy.transactions.state.forbidden' : 'economy.transactions.state.unavailable')"
          :description="props.controller.errorCode.value ?? undefined"
        />
        <UCard>
          <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-5 xl:items-end">
            <UFormField :label="t('economy.transactions.playerId')">
              <UInput v-model="playerId" class="w-full" />
            </UFormField>
            <UFormField :label="t('economy.transactions.accountId')">
              <UInput v-model="accountId" class="w-full" />
            </UFormField>
            <UFormField :label="t('economy.transactions.type')">
              <UInput v-model="type" class="w-full" />
            </UFormField>
            <UFormField :label="t('economy.transactions.businessKind')">
              <UInput v-model="businessKind" class="w-full" />
            </UFormField>
        ...
        <div v-if="props.controller.state.value === 'loading'" class="space-y-3">
          <USkeleton v-for="row in 5" :key="row" class="h-24 w-full" />
        </div>
        <UCard v-else-if="props.controller.transactions.value.length === 0">
          <p class="text-sm text-muted">
            {{ t('economy.transactions.state.empty') }}
          </p>
        </UCard>
        <div v-else class="space-y-3">
          <UCard v-for="transaction in props.controller.transactions.value" :key="transaction.transactionId">
              ...
              <div class="flex flex-wrap items-start justify-between gap-2">
                <div>
                  <p class="font-semibold text-highlighted">
                    ...
                    {{ transaction.transactionId }} · {{ new Date(transaction.occurredAtUtc).toLocaleString() }}
                  </p>
                </div>
            ...
            <p v-if="transaction.reason" class="mb-3 text-sm">
              {{ transaction.reason }}
            </p>
            <div class="grid gap-2 md:grid-cols-2">
              <div v-for="entry in transaction.entries" :key="entry.entryId" class="rounded-md border border-default p-3 text-sm">
                <div class="flex justify-between gap-3">
                  <code class="truncate text-xs text-muted">{{ entry.accountId }}</code><span :class="entry.side === 'Credit' ? 'text-success' : 'text-error'">{{ entry.side === 'Credit' ? '+' : '-' }}{{ formatEconomyAmount(entry.amount) }}</span>
                </div>
                <p class="mt-1 text-xs text-muted">
                  {{ t('economy.transactions.balanceAfter', { amount: formatEconomyAmount(entry.balanceAfter) }) }}
                </p>
              </div>
            </div>
          </UCard>
