...
<template>
  <section class="space-y-3" aria-labelledby="teleport-operations-heading">
    <div>
      <h2 id="teleport-operations-heading" class="text-base font-semibold text-highlighted">
        {{ t('community.teleport.operationsTitle') }}
      </h2>
      <p class="text-sm text-muted">
        {{ t('community.teleport.operationsDescription') }}
      </p>
    </div>
    <CommunityStateAlert :state="state" :subject="t('community.teleport.operationsSubject')" @retry="$emit('retry')" />
    <div v-if="state === 'loading' && operations.length === 0" class="space-y-3">
      <USkeleton v-for="row in 2" :key="row" class="h-44 w-full" />
    </div>
    <UCard v-else-if="state === 'empty'">
      <p class="text-sm text-muted">
        {{ t('community.teleport.operationsEmpty') }}
      </p>
    </UCard>
      ...
      <UCard v-for="operation in operations" :key="operation.operationId">
        <template #header>
          <div class="flex min-w-0 flex-wrap items-start justify-between gap-2">
            ...
            <UBadge :color="operationColor(operation.state)" variant="subtle">
              {{ operation.state === 'PendingReconciliation' ? t('community.teleport.pendingReconciliation') : operation.state }}
            </UBadge>
          </div>
        </template>
        <dl class="grid gap-3 text-sm sm:grid-cols-2 xl:grid-cols-4">
          ...
          <div>
            <dt class="text-muted">
              {{ t('community.teleport.targetPlayer') }}
            </dt><dd class="mt-1 break-all">
              {{ operation.targetCrossplatformId ?? t('community.teleport.noTargetPlayer') }}
            </dd>
          </div>
          <div>
            <dt class="text-muted">
          ...
          <div>
            <dt class="text-muted">
              ...
              {{ t('community.common.coordinates', { x: operation.destination.x, y: operation.destination.y, z: operation.destination.z }) }}
            </dd>
          </div>
        </dl>
      </UCard>
