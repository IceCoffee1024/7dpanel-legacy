  ...
  <UDashboardPanel id="community-cities">
    <template #header>
      <UDashboardNavbar :title="t('community.cities.title')">
        ...
        <template #right>
          <UButton color="neutral" icon="i-lucide-refresh-cw" :label="t('community.cities.refresh')" variant="outline" :loading="props.controller.citiesState.value === 'loading' || props.controller.fullCityListState.value === 'loading'" @click="emit('refresh')" />
        </template>
      </UDashboardNavbar>
    </template>
      ...
      <UContainer class="space-y-5 py-5">
        ...
        <section class="space-y-3" aria-labelledby="cities-heading">
          <div class="flex flex-wrap items-end justify-between gap-3">
            <div>
              <h2 id="cities-heading" class="text-base font-semibold text-highlighted">{{ t('community.cities.listTitle') }}</h2>
              <p class="text-sm text-muted">{{ t('community.cities.listDescription') }}</p>
            </div>
            <UButton color="neutral" :label="t('community.cities.create')" variant="outline" @click="clearSelection" />
          </div>
          ...
          <UCard v-else-if="props.controller.fullCityListState.value === 'empty'">
            <p class="text-sm text-muted">{{ t('community.cities.listEmpty') }}</p>
          </UCard>
            ...
            <UCard v-for="city in props.controller.fullCities.value" :key="city.cityId">
              <template #header>
                <div class="flex min-w-0 items-start justify-between gap-2">
                  <div class="min-w-0"><h3 class="font-semibold text-highlighted">{{ city.name }}</h3><p class="break-all text-xs text-muted">{{ city.cityId }}</p></div>
                  <UBadge :color="city.enabled ? 'success' : 'neutral'" variant="subtle">{{ city.enabled ? t('community.common.enabled') : t('community.common.disabled') }}</UBadge>
                </div>
              </template>
              <p class="text-sm">{{ city.description || t('community.cities.noDescription') }}</p>
              <p class="mt-2 text-xs text-muted">{{ city.position.worldId }} · {{ t('community.common.coordinates', { x: city.position.x, y: city.position.y, z: city.position.z }) }} · {{ t('community.cities.sortOrder', { value: city.sortOrder }) }}</p>
              <template #footer><div class="flex justify-end"><UButton color="neutral" :label="t('community.common.edit')" variant="outline" @click="select(city)" /></div></template>
            </UCard>
          </div>
        </section>
