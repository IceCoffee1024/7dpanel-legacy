...
function requestDelete(backup: BackupRecord) {
  if (window.confirm(t('backups.delete.confirm', { kind: t(`backups.kind.${backup.kind}`) })))
  ...
  <UDashboardPanel id="backups">
    <template #header>
      <UDashboardNavbar :title="t('backups.title')">
        ...
        <template #right>
          <UButton color="neutral" :disabled="controller.isMutating.value || policyController.isSaving.value" icon="i-lucide-refresh-cw" :label="t('common.reload')" variant="outline" @click="refresh" />
        </template>
      </UDashboardNavbar>
    </template>
    ...
    <template #body>
      <UContainer class="space-y-5 py-5">
        ...
        <UAlert v-if="controller.errorCode.value" color="error" :description="controller.errorCode.value" :title="t('backups.state.operationFailed')" />
