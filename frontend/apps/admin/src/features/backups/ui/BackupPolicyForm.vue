  ...
  <UCard>
    <template #header>
      <div class="flex flex-wrap items-center justify-between gap-2">
        <h3 class="font-semibold">{{ t(`backups.kind.${draft.kind}`) }}</h3>
    ...
    <UForm :state="state" :validate="validate" class="space-y-4" @submit="submit">
      ...
      <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        ...
        <UFormField name="retentionCount" :label="t('backups.policies.field.retentionCount')" required>
          <UInputNumber v-model="state.retentionCount" class="w-full" :disabled="disabled" :min="0" />
        </UFormField>
        <UFormField name="retentionDays" :label="t('backups.policies.field.retentionDays')" required>
          <UInputNumber v-model="state.retentionDays" class="w-full" :disabled="disabled" :min="0" />
        </UFormField>
      ...
      <div class="flex justify-end">
        <UButton type="submit" :label="t('backups.policies.save')" :disabled="disabled" :loading="disabled" />
      </div>
    </UForm>
  </UCard>
</template>
