...
<template>
  <PlayerActionDialogFrame :open="open" :target="target" :target-valid="targetValid" :pending="pending" :feedback="feedback" :can-submit="confirmed" :title="t('players.profile.actions.resetSkills.title')" :description="t('players.profile.actions.resetSkills.description')" :confirm-label="t('players.profile.actions.resetSkills.confirm')" @close="emit('close')" @confirm="emit('submit', { dangerConfirmed: true })">
    <UAlert color="warning" :title="t('players.profile.actions.resetSkills.impact')" />
