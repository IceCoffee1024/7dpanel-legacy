...
<template>
  <section class="space-y-3" aria-labelledby="player-skills-title">
    <div class="flex flex-wrap items-center justify-between gap-2">
      <h2 id="player-skills-title" class="font-semibold text-highlighted">
        {{ t('players.profile.skills.title') }}
      </h2>
    ...
    <template v-if="section.value">
      ...
      <dl class="grid grid-cols-2 gap-3 rounded-lg border border-default p-3">
        <div>
          <dt class="text-sm text-muted">
            {{ t('players.fields.level') }}
          </dt><dd>{{ section.value.level ?? t('players.profile.skills.unknown') }}</dd>
        </div>
        <div>
          <dt class="text-sm text-muted">
            {{ t('players.fields.skillPoints') }}
          </dt><dd>{{ section.value.skillPoints ?? t('players.profile.skills.unknown') }}</dd>
        </div>
      </dl>
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="border-b border-default text-left text-muted">
              <th class="p-2">
                {{ t('players.profile.skills.skill') }}
              </th><th class="p-2">
                {{ t('players.profile.skills.value') }}
              </th><th class="p-2">
                {{ t('players.fields.state') }}
              </th>
            </tr>
          </thead><tbody>
            <tr v-for="skill in section.value.values" :key="skill.skillKey" class="border-b border-muted">
      ...
      <ul class="space-y-2 md:hidden">
        <li v-for="skill in section.value.values" :key="skill.skillKey" class="rounded-lg border border-default p-3">
          <p class="break-all font-mono text-sm">
            {{ skill.skillKey }}
          </p><p>{{ valueLabel(skill) }}</p>
        </li>
      </ul>
    </template>
