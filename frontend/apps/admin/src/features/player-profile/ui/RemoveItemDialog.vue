...
watch(() => props.open, (open) => {
  ...
  quantity.value = 1
  quality.value = null
  removalMode.value = 'Exact'
})
function submit() {
  if (canSubmit.value && props.catalogVersion)
    emit('submit', { resourceId: resourceId.value.trim(), quantity: quantity.value, quality: quality.value, removalScope: 'BagOnly', removalMode: removalMode.value, catalogVersion: props.catalogVersion })
}
</script>
...
<template>
  <PlayerActionDialogFrame
    ...
    :title="t('players.profile.actions.remove.title')"
    :description="t('players.profile.actions.remove.description')"
    :confirm-label="t('players.profile.actions.remove.confirm')"
    @close="emit('close')"
    @confirm="submit"
  >
    <UForm :state="{ resourceId, quantity, quality, removalMode }" class="space-y-3">
      ...
      <UFormField :label="t('players.profile.actions.remove.mode')">
        <USelect v-model="removalMode" :items="[{ label: t('players.profile.actions.remove.exact'), value: 'Exact' }, { label: t('players.profile.actions.remove.upToAvailable'), value: 'UpToAvailable' }]" />
      </UFormField>
