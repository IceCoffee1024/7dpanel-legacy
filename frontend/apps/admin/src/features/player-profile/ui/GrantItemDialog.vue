...
watch(() => props.open, (open) => { if (open) { resourceId.value = ''; quantity.value = 1; quality.value = null; hiddenItemConfirmed.value = false } })
function submit() { if (canSubmit.value && props.catalogVersion) emit('submit', { resourceId: resourceId.value.trim(), quantity: quantity.value, quality: quality.value, hiddenItemConfirmed: hiddenItemConfirmed.value, catalogVersion: props.catalogVersion }) }
</script>

<template>
  <PlayerActionDialogFrame :open="open" :target="target" :target-valid="targetValid" :pending="pending" :feedback="feedback" :can-submit="canSubmit" :title="t('players.profile.actions.grant.title')" :description="t('players.profile.actions.grant.description')" :confirm-label="t('players.profile.actions.grant.confirm')" @close="emit('close')" @confirm="submit">
