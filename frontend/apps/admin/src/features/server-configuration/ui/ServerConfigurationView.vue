  ...
  <UDashboardPanel id="server-configuration">
    <template #header>
      <UDashboardNavbar :title="t('serverConfiguration.title')">
        ...
        <template #right>
          <UButton icon="i-lucide-refresh-cw" color="neutral" variant="ghost" :loading="controller.isRefreshing.value" :label="t('common.reload')" @click="controller.refresh" />
        </template>
      </UDashboardNavbar>
    </template>
    <template #body>
      ...
      <div v-else class="space-y-5">
        ...
        <section v-for="[group, fields] in groupedFields" :key="group" class="space-y-2">
          ...
          <article v-for="field in fields" :key="field.key" class="grid gap-3 rounded-lg border border-default p-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
            ...
            <UButton v-if="field.editable" :data-testid="`edit-${field.key}`" color="neutral" variant="outline" icon="i-lucide-pencil" :label="t('common.edit')" @click="edit(field)" />
          </article>
        </section>
      </div>
    </template>
    ...
    <template #body>
      <div v-if="selected" class="space-y-4">
        ...
        <div class="flex justify-end gap-2">
          <UButton color="neutral" variant="outline" :label="t('common.cancel')" @click="selected = null" />
