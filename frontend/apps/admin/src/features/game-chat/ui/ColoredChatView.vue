        ...
        <div class="space-y-4 pt-4">
          <div class="flex flex-wrap items-end justify-between gap-3">
            <UFormField :label="t('gameChat.colored.profiles.filterLabel')" name="profileFilter" class="min-w-64 flex-1">
              <div class="flex gap-2">
                <UInput
                  ...
                  class="w-full"
                  :placeholder="t('gameChat.colored.profiles.filterPlaceholder')"
                />
                <UButton
            ...
            <UButton
              ...
              icon="i-lucide-plus"
              :label="t('gameChat.colored.profiles.create')"
          ...
          >
            <template #actions>
              <UButton
                v-if="profilesState === 'failed'"
                :label="t('gameChat.common.retry')"
                color="neutral"
                variant="outline"
                @click="emit('retryProfiles')"
              />
            </template>
          </UAlert>
              ...
              <li v-for="profile in profiles" :key="profile.crossplatformId" class="flex flex-wrap items-start justify-between gap-4 py-4">
                <div class="min-w-0 flex-1 space-y-2">
                  ...
                  <p class="whitespace-pre-wrap wrap-break-word text-sm">
                    {{ profile.customName ?? '{playerName}' }}
                  </p>
                  <p v-if="profile.description" class="whitespace-pre-wrap wrap-break-word text-xs text-muted">
                    {{ profile.description }}
                  </p>
        ...
        >
          ...
          <section class="grid gap-4 rounded-lg border border-default p-4 md:grid-cols-2 xl:grid-cols-3">
            <UFormField :label="t('gameChat.colored.defaults.globalColor')" name="globalDefaultColor" :hint="t('gameChat.common.mayBeEmpty')">
              <div class="space-y-2">
                <UColorPicker :model-value="toChatColorPickerValue(settingsDraft.globalDefaultColor)" format="hex" @update:model-value="settingsDraft.globalDefaultColor = $event ?? ''" /><UInput
                  v-model="settingsDraft.globalDefaultColor"
                  data-testid="global-default-color-input"
            ...
            <UFormField :label="t('gameChat.colored.defaults.whisperColor')" name="whisperDefaultColor" :hint="t('gameChat.common.mayBeEmpty')">
              <div class="space-y-2">
                <UColorPicker :model-value="toChatColorPickerValue(settingsDraft.whisperDefaultColor)" format="hex" @update:model-value="settingsDraft.whisperDefaultColor = $event ?? ''" /><UInput v-model="settingsDraft.whisperDefaultColor" class="w-full font-mono" placeholder="RRGGBB" />
              </div>
            </UFormField>
            <UFormField :label="t('gameChat.colored.defaults.friendsColor')" name="friendsDefaultColor" :hint="t('gameChat.common.mayBeEmpty')">
              <div class="space-y-2">
                <UColorPicker :model-value="toChatColorPickerValue(settingsDraft.friendsDefaultColor)" format="hex" @update:model-value="settingsDraft.friendsDefaultColor = $event ?? ''" /><UInput v-model="settingsDraft.friendsDefaultColor" class="w-full font-mono" placeholder="RRGGBB" />
              </div>
            </UFormField>
            <UFormField :label="t('gameChat.colored.defaults.partyColor')" name="partyDefaultColor" :hint="t('gameChat.common.mayBeEmpty')">
              <div class="space-y-2">
                <UColorPicker :model-value="toChatColorPickerValue(settingsDraft.partyDefaultColor)" format="hex" @update:model-value="settingsDraft.partyDefaultColor = $event ?? ''" /><UInput v-model="settingsDraft.partyDefaultColor" class="w-full font-mono" placeholder="RRGGBB" />
              </div>
            </UFormField>
            <UFormField :label="t('gameChat.colored.defaults.adminColor')" name="adminDefaultColor" :hint="t('gameChat.common.mayBeEmpty')">
              <div class="space-y-2">
                <UColorPicker :model-value="toChatColorPickerValue(settingsDraft.adminDefaultColor)" format="hex" @update:model-value="settingsDraft.adminDefaultColor = $event ?? ''" /><UInput v-model="settingsDraft.adminDefaultColor" class="w-full font-mono" placeholder="RRGGBB" />
              </div>
            </UFormField>
            <UFormField :label="t('gameChat.colored.defaults.systemColor')" name="systemDefaultColor" :hint="t('gameChat.common.mayBeEmpty')">
              <div class="space-y-2">
                <UColorPicker :model-value="toChatColorPickerValue(settingsDraft.systemDefaultColor)" format="hex" @update:model-value="settingsDraft.systemDefaultColor = $event ?? ''" /><UInput v-model="settingsDraft.systemDefaultColor" class="w-full font-mono" placeholder="RRGGBB" />
              </div>
            </UFormField>
          </section>
          ...
          />
          <p v-if="settingsFeedbackMessage" role="status" class="text-sm text-error">
            {{ t(settingsFeedbackMessage) }}
          </p>
