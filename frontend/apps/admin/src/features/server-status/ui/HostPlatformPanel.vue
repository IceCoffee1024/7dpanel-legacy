...
const rows = computed(() => [
  ...
  ['osFamily', props.host.osFamily], ['architecture', props.host.operatingSystemArchitecture], ['runtime', props.host.runtimeVersion],
  ['cpu', props.host.cpuModel], ['cores', props.host.logicalCoreCount ?? props.host.processorCount],
...
<template>
  <UCard class="rounded-md">
    <template #header><h2 class="font-semibold text-highlighted">{{ t('overview.hostPlatform.title') }}</h2></template>
    <dl class="grid gap-x-6 gap-y-3 sm:grid-cols-2">
      <div v-for="row in [...rows, ...sensitiveRows]" :key="row[0]" class="min-w-0"><dt class="text-xs text-muted">{{ t(`overview.hostPlatform.${row[0]}`) }}</dt><dd class="mt-1 break-all text-sm text-highlighted">{{ row[1] ?? '—' }}</dd></div>
    </dl>
  </UCard>
</template>
