...
test('Owner completes the mock J2 discovery, fixed-target kick, history, map, and audit journey', async ({ page }) => {
  ...
  await page.getByRole('button', { name: /查看.*详情|view.*details/iu }).first().click()
  await expect(page.getByRole('dialog')).toContainText(player.crossplatformIdentity.combinedId)

  await page.getByRole('link', { name: /查看历史|view history/iu }).click()
  await expect(page).toHaveURL(new RegExp(`/players/history/${crossplatformId}$`))
  await expect(page.getByText(player.name).first()).toBeVisible()

  await gotoAdmin(page, '/players')
  await page.getByRole('button', { name: /查看.*详情|view.*details/iu }).first().click()
  await page.getByRole('link', { name: /查看地图|view map/iu }).click()
  ...
  await gotoAdmin(page, '/players')
  await page.getByRole('button', { name: /踢出.*Browser Journey Player|kick.*Browser Journey Player/iu }).first().click()
