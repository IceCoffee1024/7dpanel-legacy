...
|---|---|---|---:|---|---|
| User Journey | 单一前端 Feature/页面及其局部测试 | Job/Restore、OpenAPI、generated SDK、共享导航事实源（未经协调） | 0 | pending | pending |
| Developer Path | 单一调用切片及其聚焦测试 | User/Recovery 已占用文件、物理项目边界 | 0 | pending | pending |
| Failure Recovery | 单一 operation/job 错误投影及测试 | Restore 安全机制、migration、API 契约（未经协调） | 0 | pending | pending |
| Governance | 本文件、贡献/检查脚本及其 self-test | 生产行为、预算阈值、成熟度台账 | 0 | pending | pending |
...
> 由协调者在代码修改前补充当前命令结果、候选选择、代理迭代和独立审查。

## 决策日志

| ID | Stream | 假设 | 证据 | 决策 | 回滚/复核触发 |
|---|---|---|---|---|---|
| OUT-001 | User Journey | pending | pending | pending | pending |
| OUT-002 | Developer Path | pending | pending | pending | pending |
| OUT-003 | Failure Recovery | pending | pending | pending | pending |
| OUT-004 | Governance | pending | pending | pending | pending |
