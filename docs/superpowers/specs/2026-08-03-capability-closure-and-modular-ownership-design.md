...
- 后端当前有八个产品项目和 583 个生产 `.cs` 文件，其中业务变化经常横跨 Application、多个 Adapter、Bootstrap、OpenAPI、Admin 与多层测试。
- 单一后端测试项目当前包含 197 个测试源文件；项目边界简单，但按能力检索和聚焦执行的成本正在上升。
- `PanelServiceProviderFactory.cs` 已达到 1433 行，单一组合根正确，但注册清单缺少能力级导航。
- Task 10 已清除 Hosting -> Application 项目引用；后续必须保持技术中立边界并防止平台采样、脚本或业务用例重新汇入 Hosting。
...
本次重基线把后续工作纳入同一个候选版本收口批次，但仍按依赖拆成可独立验证和回滚的波次。下表只定义目标范围，不表示实现或验证已经完成。

| ID | 必须模块 | 简要目标 |
|---|---|---|
| M1 | 验证环境补齐 | 提供受控 7DTD 引用、Playwright Chromium、Windows/Linux 实例和隔离测试资源 |
| M2 | 自动化基线闭环 | 完成后端聚合、能力/边界过滤、Admin 聚合和治理门禁 |
| M3 | Bootstrap 组合根收口 | 按 Platform/Capability 拆分注册清单，同时保持单一 Provider 和原生命周期 |
| M4 | 核心旅程闭环 | 在当前候选 artifact 上完成 J1、J2、J3 |
| M5 | 恢复与回滚闭环 | 证明一致性、中断续作、失败重试、安全副本、回滚和审计 |
| M6 | 用户任务体验收口 | 补齐渐进披露、角色与模块感知入口、高风险确认和核心任务连续性 |
| M7 | 候选发布物冻结 | 生成唯一 artifact identity、内容清单和 evidence manifest |
| M8 | 真实环境验证 | 覆盖 Windows/Linux 7DTD、Chromium、真实玩家和 P0 游戏副作用 |
| M9 | 安全与可靠性门禁 | 覆盖认证授权、敏感信息、队列、排空、异常终止和未知结果 |
| M10 | 性能基线与阈值 | 从官方双平台进程建立可执行的主线程、队列、API 和负载阈值 |
| M11 | 成熟度与发布判定 | 依据冻结证据更新 Current 文档，并形成唯一发布结论 |

以下增强模块也纳入本批次，但不改变 `docs/test.md` 已定义的 P0 发布门槛。环境不具备或验证失败时保留明确 blocker，不得用替身、旧 artifact 或跳过结果补证：

| ID | 增强模块 | 简要目标 |
|---|---|---|
| O1 | `CAP-08` 至 `CAP-12` 扩大验收 | 在不干扰 P0 的独立资源上尽可能验证玩家资源、经济奖励、社区高级能力和世界工具 |
| O2 | Discord 与 GeoIP 外部服务验收 | 在 sandbox 或受控真实服务验证连接、Secret 轮换、限流和故障恢复 |
| O4 | 私有 CI 与受控 Runner | 复用同一门禁自动验证私有游戏引用、双平台候选进程和 Chromium |

未选择的 O3 浏览器扩展矩阵和 O5 渐进式物理归拢不进入本批次。Firefox/WebKit 需先形成产品支持决定；除 Hosting 边界修正和 Bootstrap 注册清单拆分外，不移动生产目录或命名空间。

## 非目标

- 不通过减少项目数量、合并层级或移动全部文件来追求表面简洁。
- 不把 583 个生产文件一次性重排为新的物理目录树，不进行全仓库命名空间改写。
...
- 不在产品支持范围确定前把 Firefox/WebKit 纳入发布门槛；本阶段以 Chromium 稳定版作为浏览器基线。
- 不在本阶段进行 Capability 目录迁移；逻辑所有权和依赖规则先于物理布局。
...
- 保持概览、服务器运维、玩家、社区、经济与奖励、自动化、系统管理的既有任务分组，不再增加一级入口。
- 默认只呈现高频和安全操作；高级设置、诊断和危险操作按角色、模块状态与任务上下文渐进披露。
- 无权或模块未启用的入口在导航和上下文操作中隐藏，直接访问仍由路由和服务端授权独立拒绝。
- J1、J2、J3 分别形成连续任务流程，用户不需要自行串联技术页面；失败、断线和刷新后仍能回到权威状态。
- 面板角色、游戏原生权限和 Capability 授权保持独立概念，界面不得暗示自动映射。
...
`CAP-08` 至 `CAP-12` 纳入本批次的扩大验收，但不因被纳入或候选发布自动升级。每项只按实际取得的当前 artifact、真实游戏、浏览器或外部服务证据更新为 `Implemented`、`Verified` 或 `Blocked`；未闭环项不阻塞现有 P0 候选发布，除非其失败暴露共享安全、数据损坏、恢复或运行时风险。只有宣称旧版主要功能完全对齐时，才要求 `docs/test.md` 已规定的全部 P1 门禁通过。
...
- 私有 CI/Runner 只自动化本规格和 `docs/test.md` 已定义的门禁，不创建第二套测试、artifact hash 或成熟度规则。
- 游戏程序集、外部服务凭据和受控环境配置由 Runner 环境提供，不进入仓库、日志、缓存 artifact 或公开构建。
- Runner 不可用时允许按同一入口人工执行并保留 manifest；这会阻止相应自动化目标完成，但不改变已有 P0 合同和证据要求。
...
| Wave | 目标 | 主要输出 | 依赖 |
|---|---|---|---|
| 0. 环境与基线 | 固定范围、状态、证据格式和可执行环境 | 受控引用与 Runner、能力台账、当前 blocker、复杂性基线、净新增能力冻结 | 无 |
| 1. 自动化闭环 | 让现有代码与守护规则形成可信基线 | 后端/Admin 聚合、测试 filter、依赖/成熟度门禁和 evidence manifest | Wave 0 |
| 2A. 服务器运维 | 完成 J1 和相关任务体验 | Windows/Linux 启停、重启、operation 恢复、当前认证/Swagger、审计证据 | Wave 1 |
| 2B. 玩家管理 | 完成 J2 和相关任务体验 | 真实玩家字段、历史、浏览器、踢出和审计闭环 | Wave 1，可与 2A 并行但使用独立实例 |
| 2C. P0 共享验证 | 收口 `CAP-04` 至 `CAP-07` | 自动化、认证、配置/Mod、聊天和真实浏览器/游戏最小矩阵 | Wave 1，可与 2A/2B 并行 |
| 2D. 扩大验收 | 执行 O1 与 O2 | `CAP-08` 至 `CAP-12`、Discord/GeoIP 的适用真实证据或明确 blocker | Wave 1，可与 2A/2B/2C 使用独立资源并行 |
| 3. 备份恢复 | 完成 J3 | 一致性、文件时机、故障注入、Windows/Linux 跨重启恢复与回滚证据 | Wave 2A 的可靠启停与证据链 |
| 4. 结构收口 | 降低横向阅读成本 | Bootstrap 注册清单按能力拆分、组合根 characterization 与依赖门禁 | Wave 1；Hosting 收口结果保持不回归 |
| 5. 候选发布 | 证明首版发布门槛并接入 O4 | P0/NFR 聚合、性能阈值、双平台、Chromium、恢复、回滚、私有 Runner、文档审计和候选 artifact | Waves 2 至 4 |

Wave 2A、2B、2C、2D 只能在独立测试实例和独立证据目录上并行；不得并发操作同一个 7DTD 进程、世界、端口、SQLite 数据库或外部服务账号。Wave 3 的破坏性恢复演练只使用可销毁世界，并在执行前确认备份、回滚目标和停止条件。
...
O1、O2、O4 的完成情况必须在最终结论中单独列出。它们未完成时保持明确原因和后续条件；除非发现影响 P0 的共享风险，否则不把增强目标转换为新的首版发布门槛。

本规格由[现有唯一实施计划](../plans/2026-08-03-capability-closure-and-modular-ownership.md)执行；计划保留已完成任务的证据，只重排未完成工作，并把每个 Wave 拆为可验证任务、明确并行边界、真实环境前置条件、停止条件和文档提升时机。
