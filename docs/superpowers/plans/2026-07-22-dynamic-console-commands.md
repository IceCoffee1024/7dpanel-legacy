...
- [ ] **步骤 4：运行 Application 测试并验证 GREEN**
...
- [ ] **步骤 3：实现最小 Patch 与 source context**
...
- [ ] **步骤 4：运行 Patch 测试并验证 GREEN**

  运行步骤 2。预期：所有来源、复制、嵌套和异常透明性测试通过。

- [ ] **步骤 5：写审计服务队列与 fail-open 失败测试**
...
- [ ] **步骤 6：运行审计服务测试验证 RED，补齐实现后验证 GREEN**
...
- [ ] **步骤 2：运行端到端测试并验证 RED，修复装配缺口后验证 GREEN**
...
- [ ] **步骤 4：执行发布物检查**
  ...
  ```powershell
  backend/scripts/Publish-Mod.ps1 -Configuration Release
  ```
