...
| Admin Vitest teardown 存在未定位噪声 | `pnpm test` 的 `31` 个文件、`335/335` 项断言通过，但输出仍出现 happy-dom `AbortError` 和外部连接拒绝日志。当前未能确认噪声来源；在根因和稳定修复完成前，它不构成干净的 CI 或发布证据。 |
