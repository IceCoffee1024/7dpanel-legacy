...
本文档定义首版产品合同、[界面设计](design.md)和[系统架构](architecture.md)风险的验证方式。当前后端解决方案包含六个产品项目和一个迁移保护测试项目；启用 C# `11.0`、Nullable Reference Types 和 Implicit Usings。无 Basic/API Key 认证变更后的 Release 聚合构建和全量 xUnit 已有 331 项通过，覆盖 password grant、8 小时默认 Access Token、Access Token/API Key 严格分流、当前用户/角色重建、API Key REST/OpenAPI、Basic 拒绝、SSE 周期复验、事件投影、输入验证与依赖边界；当前工作树也以显式 `SevenDaysReferenceRoot` 完成 Release 还原、构建和全量 xUnit，331 项通过，覆盖 password grant 成功响应的服务端 `username`/`role` 与 Admin 文档 CSP。Admin 的 Auth 与 locale codec/Repository/runtime、身份响应解析、登录/账号/语言菜单、全部当前页面双语、Valibot 表单边界和路由定向自动化已通过；完整 `pnpm test` 为 29 个文件、318 项通过，lint、typecheck 和 Vite 8 生产构建均通过。消息目录自动化要求中英文叶子键和插值参数完全一致、值非空且不含 HTML；生产源码扫描未发现目录外的中文产品文案。该 Vitest 输出仍包含 happy-dom teardown 的 `AbortError`、fork worker 终止超时和外部连接 `ETIMEDOUT`，根因未确认，不能视为干净的测试基础设施。当前 Owner Playwright suite 已有 12 项场景并通过测试发现，其中 4 项覆盖浏览器语言首选与英文回退、登录前切换、刷新持久化、登录/登出保留、技术身份稳定和英文窄屏；由于没有受控 OWIN 环境变量，它尚未执行，历史 Edge 结果不能替代本实现。2026-07-23 的远程 Windows 7DTD `v3.0.1-b4` 闭环 smoke 验证了合并提交 `a98ad6b` 的公开 Swagger UI/OpenAPI Unity Mono 加载与访问、Telnet 原生异步队列、真实 SQLite 写锁期间命令 fail-open、解锁后的 `store_failure` gap 恢复、正常关服排空、进程/监听释放和重复启停；该动态命令与 Swagger 证据发生在移除 Basic 前，不能证明当前认证版本的真实进程行为。当前认证版本的事件投影真实 Join/Save/Disconnect、Owner 踢出、浏览器持久会话、自动归档和 Linux 真实进程仍未验证。完整用户管理、其他状态变更游戏动作和其他产品能力仍未实现。以下未落地内容仍是目标测试策略和发布门槛，不代表测试已经通过。
...
| `CAP-02` | 在线玩家 25 字段同次观察、逐玩家时间、主表与详情抽屉；踢出、禁言、封禁和传送；动态内置/第三方控制台命令；HTTP 有界 FIFO 与主线程串行；日志过滤；成功、失败、拒绝和审计缺口 | 单元、SQLite 集成、OWIN/API 集成、真实进程、E2E、安全 | 游戏内结果、API 结果、详情状态、队列顺序、Harmony 覆盖和对应审计记录 |
...
| 在线玩家字段、事件或投影边界漂移 | 自动化必须验证 Join membership、一次 Save 对 25 字段的不可变复制和共同 `observedAtUtc`、Disconnect 身份条件删除、排序、设备枚举、Discord 十进制字符串、综合权限、可空字段、单位后缀、停止拒写与清空、旧 observation 不被服务端年龄策略拒绝、Owner-only、就绪短路和精确字段白名单；只有真实测试玩家 smoke 才能证明 `v3.0.1-b4` 事件注册、字段值和单位兼容，任何游戏活对象不得进入投影或离开事件回调。 |
...
- 在线玩家详情切片实施时，单元测试必须验证 Application 25 字段快照、产品自有位置值、逐玩家观察时间和用例转发，以及 Adapter 的 Join/Save/Disconnect 顺序、同一次 Save 的完整不可变复制、稳定排序、设备枚举、Discord `0`/大整数、综合权限与默认 `1000`、生命浮点向零截断、可空诊断字段、身份条件删除、停止拒写与清空、非有限或无效 Save 保留旧 observation、任意年龄 observation 原样返回和缺少 observation 时不产生占位玩家；事件注册、字段值和单位兼容仍由官方进程 smoke 负责。
- 在线玩家详情切片实施时，Admin Players Feature 测试必须验证完整 25 字段 parser、位置和设备类型、所有空值统一显示“未知”、坐标与距离按当前语言四舍五入为整数、分钟值格式化，以及桌面表格/移动列表打开同一详情抽屉。组件状态必须覆盖同 entity ID 与原生身份刷新时更新详情、`A 在线 -> A 缺失 -> A 同身份重现` 仍锁存 unavailable、身份变化时保留最后 observation、Stale/Offline/Forbidden/Session expired/game-not-ready 禁用详情踢出、关闭后重新建立新目标，以及详情刷新不能替换已经固定的踢出目标。
...
- 在线玩家详情切片实施时，同一 Katana 主机必须验证玩家查询匿名 401、Owner 空/多玩家 200、根对象仅含 `players`、每个玩家固定 25 个 camelCase 字段、结构化位置、设备枚举、Discord/跨平台身份/诊断字符串空值、合法 UTC `observedAtUtc`、单位后缀与排序、游戏未就绪时不调用 Query，以及旧 observation 仍以原时间成功返回；在线玩家端点不拥有 busy、主线程启动超时或投影 stale 503。当前持久认证只创建 `Owner`，非 Owner 403 随角色管理切片验证。
...
- 在线玩家详情浏览器门禁使用合成 25 字段响应分别在桌面、`390x844` 和 320 CSS 像素宽度打开抽屉，验证四个分区、整数坐标/距离、可读分钟值、统一“未知”、复制按钮可访问名称、焦点进入与返回、抽屉内滚动和页面无水平溢出；再通过连续响应验证同身份更新、离线保留与身份复用不偷换目标。真实字段值兼容仍由 7DTD 进程测试负责，浏览器 stub 不替代该证据。
...
| 当前浏览器持久会话、双语与踢出真实门禁未执行 | 当前 12 项 Playwright 场景已经定义默认标签页会话、显式浏览器会话、身份显示、登出/到期/损坏记录清理、既有 API Key/玩家合同，以及浏览器语言首选/回退、登录前后切换、刷新与登出保留、技术身份稳定和英文窄屏，但没有 `SEVENDPANEL_ADMIN_URL`、`PANEL_USERNAME`、`PANEL_PASSWORD` 的受控 OWIN 环境，只有测试发现证据。真实双语布局、踢出、`390x844` 踢出对话框、CSP 控制台、键盘/遮罩关闭锁定或成功刷新也未执行，不能从组件或定向 Vitest 推导。 |
| Admin Vitest teardown 存在未定位噪声 | `pnpm test` 的 29 个文件、318 项断言通过，但输出仍出现 happy-dom `AbortError`、fork worker 终止超时和外部连接 `ETIMEDOUT`。API Key 与 locale 视图测试已 mock 业务请求，当前未能确认噪声来源；在根因和稳定修复完成前，它不构成干净的 CI 或发布证据。 |
