...
- 覆盖初始化凭证字符集与四位分组、大小写和连字符规范化、30 分钟边界、单次消费、并发创建、重新生成替换旧状态和已有 `Owner` 时拒绝生成，以及权限矩阵、状态新鲜度、统一错误映射、幂等键、调度规则、备份/恢复状态机和会话策略。
...
当前 `backend/` 和 `frontend/apps/admin/` 尚无工程文件，因此没有可声明为权威且可运行的应用构建、测试或 E2E 命令。工程初始化时必须在本节和根 `README.md` 同步登记实际命令，至少包括：
...
- 发布物路径和清单校验，确认 `7dtd-reference/` 不会被复制或打包。
