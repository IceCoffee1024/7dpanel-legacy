...
- 当前 Katana 主机验证控制台命令匿名 401、Owner/Admin 原文与 actor 透传、畸形 JSON/字段类型错误的 400 `invalid_request_body`、成功绑定后空白命令的 `console_command_required`、游戏未就绪 503、队满/服务停止 503、主线程启动超时、取消和并发请求独立输出；匿名非法 body 仍由认证优先返回 401。SevenDays Adapter 的可控 Dispatcher 测试验证严格 FIFO、等待容量和队满拒绝。无效请求、队满和未就绪路径不会执行命令；未知命令不再由 Web 白名单拒绝，而由 7DTD 返回真实输出。
