...
| 有界日志与命令通道 | `System.Threading.Channels 8.0.0`、`System.Threading.Tasks.Extensions 4.6.3` | 控制台日志、HTTP 命令 FIFO、异步审计自动化和本地发布清单通过；`a98ad6b` 已在 Windows Mono 验证原生异步队列、SQLite 写锁 fail-open/gap 恢复、关服排空和重复启停。当前认证版本尚未重复该真实进程检查 | 三条通道容量和生命周期独立；发布 Channels、Tasks.Extensions 和 Unsafe，仍不复制 Harmony、LogLibrary 或 Unity 程序集；Linux Mono 仍待验证 |
