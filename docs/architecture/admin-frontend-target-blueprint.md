...
它覆盖 `CAP-01` 至 `CAP-12`、`NFR-01` 至 `NFR-04` 的前端实现边界，
但不重新定义产品行为或界面设计。

旧版页面仅作为行为证据；固定提交、能力处置与开发顺序由[旧版本功能对齐目标蓝图](legacy-feature-parity-target-blueprint.md)记录。Admin 继续使用本蓝图的 Feature、状态和 Nuxt UI 边界，不复制旧版页面结构或组件代码。
...
| `frontend/apps/player/` | 未来玩家网页身份与玩家网页商城 | 产品立项后独立应用和认证边界 | 否，当前不创建；`CAP-09` 的游戏内经济与商店仍由 Admin 配置、由游戏身份使用 |
...
- `features/` 按身份、概览、玩家、玩家档案与物品、地图与世界、日志与聊天、访问名单、服务器配置、模组与功能模块、权限、公告与自动化、备份、经济与奖励、商店与兑换、传送与投票、审计与游戏事件、Discord、GeoIP 和设置组织；每个 Feature 拥有自己的 API 映射、
  状态转换、组件和测试。
...
| 游戏资源目录 | 后端版本化不可变 snapshot | 页面局部 `useGameResources` 持有 URL filters、请求取消、Building retry、最后成功页和 Stale/Partial 状态；图标组件持有短生命周期 Blob URL | 复制到 Pinia/Storage、把 Token 放入 URL、把目录包装成全局通用缓存 |
